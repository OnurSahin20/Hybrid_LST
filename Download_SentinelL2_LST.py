...
1) It creates parent folders (dates) and child folders (ascending,descending products) for Sentinel 3A 
   and downloads each days LST to the right folders.
"""
...
class SentinelLST:
    ...
    def open_search_query(self):  # it returns to request get object.
        ...
        for i in range(len(self.time_range) - 1):
            date = 'beginposition:' + "[" + self.time_range[i] + "T00:00:00.000Z" + " TO " + self.time_range[i + 1] + \
                   "T00:00:00.000Z" + "]"
            all_queries.append(self.url_start + self.platform + " AND " + date + " AND " + loc +
                               " AND " + self.file_name + '&orderby=ingestiondate desc')
        return all_queries
    ...
    def download_day(self, index) -> None:
        ...
        [os.makedirs(par_dir + "\\" + orbit, exist_ok=True) for orbit in ["ascending", "descending"]]
        ...
        for i in range(len(down_links)):
            ...
            with open(par_dir + "\\" + direction[i] + "\\" + name + ".zip", "wb") as file:
    ...
    def extract_zip(self):
        ...
        child_direcs = ["ascending", "descending"]
        print("unzipping is started !")
        for par in par_direcs:
            x = os.listdir(par)
            if x == child_direcs:
                try:
                    for child in child_direcs:
                        p = par + "\\" + child
                        zips = os.listdir(p)
                        for z in zips:
                            zz = p + "\\" + z
                            with zipfile.ZipFile(zz, "r") as zip_obj:
                                zip_obj.extractall(path=p)
                except PermissionError:
                    raise FileExistsError("File may include not zip file. Check folders")
            else:
                raise ValueError('Only "ascending" and "descending" child folders are accepted')
...
if __name__ == '__main__':
    ...
    area = [26, 36, 27, 37]  # Rectangle of the study area (west,south,east,north)
    # Spatial bounds also can be implemented using shapefile of study area (Later release!)
    lst = SentinelLST(download_loc, us_name, pasword, period[0], period[1], area)
    lst.download_all()
    lst.extract_zip()
